...
int main(int argc, char **argv) {
  ...
  def_path = codes_definition_path(c);
    printf("path def : %s\n",def_path);
  if (!def_path) {
    printf("gribr: GRIB definition files not found; Please set the "
  ...
  const char *filename =
      "../data/"
      "W_fr-meteofrance,MODEL,AROME+001+SP1+00H_C_LFPW_202102031200--.grib2";
  codes_handle *h = NULL;
  ...
  /* get as a double*/
  CODES_CHECK(codes_get_double(h, "latitudeOfFirstGridPointInDegrees",
  ...
  /* get as a double*/
  CODES_CHECK(codes_get_double(h, "longitudeOfFirstGridPointInDegrees",
  ...
  /* get as a double*/
  CODES_CHECK(codes_get_double(h, "latitudeOfLastGridPointInDegrees",
  ...
  /* get as a double*/
  CODES_CHECK(codes_get_double(h, "longitudeOfLastGridPointInDegrees",
  ...
  /* get as a double*/
  CODES_CHECK(codes_get_double(h, "jDirectionIncrementInDegrees",
                               &jDirectionIncrementInDegrees),
              0);
  printf("jDirectionIncrementInDegrees=%g\n", jDirectionIncrementInDegrees);

  /* get as a double*/
  CODES_CHECK(codes_get_double(h, "iDirectionIncrementInDegrees",
...

  {
    ...
    if (eq != 0)
      assert(!"file and filename not equal");
  }
